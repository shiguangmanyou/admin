...
<script>
	//查询详情页数据
	$.get('/crm/workbench/clue/toDetail',{'id':'${id}'},function (data) {
		//data:clue
		//处理关联市场活动数据
		...
		for(var i = 0; i < activities.length; i++){
			...
			$('#activityBody').append("<tr>\n" +
					...
					"\t\t\t\t\t\t\t<td><a onclick=\"unbind('"+activity.id+"',$(this))\" href=\"javascript:void(0);\"  style=\"text-decoration: none;\"><span class=\"glyphicon glyphicon-remove\"></span>解除关联</a></td>\n" +
					"\t\t\t\t\t\t</tr>");
		}
	},'json');

	//给市场查询模态窗口的文本输入框添加回车事件
	$('#name').keypress(function (event) {
		if(event.keyCode == 13){
			//按名称模糊查询市场活动
			$.get("/crm/workbench/clue/queryActivities",{
				'name':$(this).val(),//输入市场活动名称
				'id' : '${id}'//线索id
			},function (data) {
				//data:List<Activity>
				$('#body').html("");
	...
	function bind() {
		...
		$('.son:checked').each(function () {
			ids.push($(this).val());//1,2,3,4
		});
		$.get("/crm/workbench/clue/bind",{
			'ids' : ids.join(),//勾中的主键的字符串
			'id' : '${id}'
		},function (data) {
			//data:result
			if(data.ok){
	...
	//解除绑定
	function unbind(id,$this) {
		$.get("/crm/workbench/clue/unbind",{
			'activityId' : id,//勾中市场活动主键
			'clueId' : '${id}'//线索id
		},function (data) {
			//resultVo
			if(data.ok){
				alert(data.message);
				//删除页面的指定的tr parent:直接的父级
