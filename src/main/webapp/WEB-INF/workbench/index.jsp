		...
		<div class="modal-dialog" role="document" style="width: 70%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" data-dismiss="modal" onclick="updateUser()">更新</button>
				</div>
			</div>
		</div>
	</div>
			...
			<ul>
				<li class="dropdown user-dropdown">
					<c:choose>
						...
						<c:otherwise>
							<a href="javascript:void(0)" style="text-decoration: none; color: white;" class="dropdown-toggle" data-toggle="dropdown">
								<img src="${user.img}" style="width: 30px;height:30px;border-radius: 100%">${user.name} <span class="caret"></span>
							</a>
						</c:otherwise>
					</c:choose>
...
<script>
	...
	//点击更新按钮，更新用户数据
	function updateUser() {
		$.post("/crm/settings/user/updateUser",{
			'loginPwd' :$('#newPwd').val(),//输入的新密码
			'img' : $('#photoPath').val()//用户头像路径
		},function (data) {
				//data:resultVo
			if(data.ok){
				layer.alert(data.message, {icon: 6})
