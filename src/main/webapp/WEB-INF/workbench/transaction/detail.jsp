...
<script>
	...
	//true/false 0/1 undefined
	refresh();

	/**
	 * js调用函数，不管有没有参数，只要函数名一样都可以调用
	 * ，如果有参数传递，把参数封装到函数中的内嵌参数数组中(arguments)
	...
	function refresh(index) {
		//异步查询交易图标数据
		$.get("/crm/workbench/tran/queryStages",{
			'id' : '${id}',//交易的id
			'index' : index//点击阶段对应的索引
		},function (data) {//服务器响应之后的结果
			...
			for(var i = 0; i < stageVos.length; i++){
				...
				if(stageVo.type == "绿圈"){
					content += "<span class=\"glyphicon glyphicon-ok-circle mystage\" onclick=\"changeStage("+i+",'"+stageVo.content+"')\"  data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stageVo.content+" style=\"color: #90F790;\"></span>";
				}else if(stageVo.type == "锚点"){
					content += "<span class=\"glyphicon glyphicon-map-marker mystage\" onclick=\"changeStage("+i+",'"+stageVo.content+"')\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stageVo.content+" style=\"color: #90F790;\"></span>";
				}else if(stageVo.type == "黑圈"){
					content += "<span class=\"glyphicon glyphicon-record mystage\" onclick=\"changeStage("+i+",'"+stageVo.content+"')\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stageVo.content+"></span>";
				}else if(stageVo.type == "红x"){
					content += "<span class=\"glyphicon glyphicon-remove mystage\" onclick=\"changeStage("+i+",'"+stageVo.content+"')\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stageVo.content+" style=\"color: red;\"></span>";
				}else{
					content += "<span class=\"glyphicon glyphicon-remove mystage\" onclick=\"changeStage("+i+",'"+stageVo.content+"')\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stageVo.content+"></span>";
				}
				content += "-----------";
			}
	...
	//点击修改交易图标
	function changeStage(index,content) {
		var arr = content.split(":");

		$('#stage').text(arr[0]);
		$('#possibility').text(arr[1]);
		//刷新阶段图标
		refresh(index);
	}
</script>
